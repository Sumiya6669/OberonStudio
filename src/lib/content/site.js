 ...
 *
 * Telegram и WhatsApp — боевые.
 *
 * Почты здесь нет намеренно. Ящика hello@tinker.kz не существует, домен не
 * куплен, письмо по такому адресу просто пропадёт. Ключ убран целиком, а не
 * оставлен пустой строкой: всё, что читает email (разметка Organization в
 * lib/seo/jsonld.js, плитки каналов в Contact.jsx), уже проверяет значение на
 * пустоту, поэтому отсутствующий ключ и пустая строка ведут себя одинаково —
 * а отсутствие ключа честнее: оно не выглядит как «адрес забыли вписать».
 * Когда ящик появится, достаточно вернуть сюда строку email.
 */
